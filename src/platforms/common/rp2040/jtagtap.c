 ...
 * Modified by Treble <bassusteur@protonmail.com>
 *
...
void jtagtap_init(void)
{
	...
	//TMS_SET_MODE();   DON'T FORGET TO FIGURE THIS OUT   DON'T FORGET TO FIGURE THIS OUT   DON'T FORGET TO FIGURE THIS OUT
...
static bool jtagtap_next_clk_delay()
{
	...
	for (volatile uint32_t counter = target_clk_divider; counter > 0; --counter)
		continue;
	const uint16_t result = gpio_get(TDO_PIN);
	gpio_put(TRST_PIN, 0);
	for (volatile uint32_t counter = target_clk_divider; counter > 0; --counter)
		continue;
	return result != 0;
...
{
	gpio_set_val(TMS_PIN, tms);
	gpio_set_val(TDI_PIN, tdi);
	...
	const uint8_t *const data_in, uint8_t *const data_out, const bool final_tms, const size_t clock_cycles)
{
	...
	for (size_t cycle = 0; cycle < clock_cycles; ++cycle) {
		...
		gpio_set_val(TMS_PIN, cycle + 1U >= clock_cycles && final_tms);
		/* Set up the TDI pin and start the clock cycle */
		gpio_set_val(TDI_PIN, data_in[byte] & (1U << bit));
	...
	const uint8_t *const data_in, uint8_t *const data_out, const bool final_tms, const size_t clock_cycles)
{
	...
	for (size_t cycle = 0; cycle < clock_cycles;) {
		...
		gpio_put(TRST_PIN, 1);
		/* If TDO is high, store a 1 in the appropriate position in the value being accumulated */
		if (gpio_get(TDO_PIN)) /* XXX: Try to remove the need for the if here */
			value |= 1U << bit;
